...
- **Los rangos de Cloudflare hay que refrescarlos, y nada avisa cuando cambian.** Un rango caduco deja de reconocerse como proxy y el tráfico que entre por él vuelve al cupo global, en silencio. `cf-connecting-ip` **no reduce este problema**: el par TCP sigue teniendo que estar declarado, y además —ver el punto siguiente— renuncia a la protección posicional. Corresponde al runbook de la 11-B, junto con el troceo a `/16` que impone `PREFIJO_MINIMO_IPV4`: cuatro de los quince rangos IPv4 de Cloudflare son más amplios que `/16` y la guardia los rechaza.
- **Las cabeceras de valor único trasladan la garantía del código al proxy, y eso hay que decirlo sin suavizar.** Con `x-forwarded-for` el valor está protegido por posición fija *y* por la validación de la cola. Con `cf-connecting-ip`, `true-client-ip` o `x-real-ip` se acepta el valor íntegro en cuanto el par TCP es de confianza: **el proxy inverso es el único control que queda**. Si no borra la cabecera en lo que entra de internet —nginx no lo hace por omisión—, el cliente elige su identidad en cada petición y el defecto que este ADR cierra reaparece completo. Escrito en `red.py` junto al código que lo hace, porque es ahí donde mira quien lo mantiene.
