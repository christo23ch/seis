...
Añadido el 2026-09-11 y ampliado el 2026-09-13, porque **ya van cinco veces que el defecto aparece en el mecanismo que debía avisar y no en lo que ese mecanismo vigilaba**. Los tres primeros son de la misma subclase; el cuarto y el quinto son subclases distintas y van explicados aparte — **y el quinto es, con diferencia, el más grave de los cinco**:
...
3. **Fase 0'.** `scripts/capturar.mjs` —el script con el que `docs/REVISION_VISUAL.md` obliga a capturar el antes y el después de **toda** pantalla que toque una fase de frontend— **estaba en un sitio desde el que no podía ejecutarse**. ESM resuelve los `import` desde el directorio del fichero, y `playwright` vive en `frontend/node_modules`: lanzado tal y como lo documentaba ese mismo apartado, moría con `ERR_MODULE_NOT_FOUND` **antes de abrir el navegador**. Es decir, la red de seguridad que exige ver el resultado en vez de suponerlo llevaba desde que se escribió **sin poder ver nada**, y es el tercer tropiezo del proyecto con esta misma causa —`frontend/e2e/alta-real.mjs` se movió por lo mismo—.
...
#### El quinto caso es el peor, y no es «otra entrada de la lista»

5. **Fase 15, `scripts/capturar.mjs`.** Las capturas de revisión visual se toman con `fullPage: true`. Esa opción **ensancha la imagen hasta el contenido**: si la página mide 889 px en un móvil de 390, `fullPage` no produce una captura de 390 px con la mitad cortada — produce una captura de 889 px **que se ve perfectamente bien**. El desbordamiento horizontal es, por construcción, **exactamente el defecto que `fullPage` vuelve invisible**.

**La diferencia con los cuatro anteriores.** Los cuatro primeros eran una herramienta de verificación fallando **sobre un caso**: un test, una mutación, un script, un token. Este es la **herramienta de captura construida de forma que oculta una CLASE ENTERA de defecto** — precisamente la clase que existía para detectar. No es que fallara una vez: es que no podía funcionar nunca para esa pregunta, y **ha estado en uso desde la Fase 0' sin que nadie lo notara.**

**Consecuencia retroactiva, y hay que asumirla explícitamente:** *toda captura «antes/después» tomada hasta hoy en cualquier fase de frontend queda como **no fiable en cuanto a desbordamiento horizontal**.* No porque esté mal archivada ni mal tomada, sino porque **el método no podía ver eso**. Cualquier conclusión de responsividad apoyada en esas capturas hay que volver a medirla; las conclusiones de contenido, composición y color siguen siendo válidas, porque para eso sí servían.

Lo que lo destapó no fue mirar mejor las capturas: fue **medir `scrollWidth`** — hacerle a la página una pregunta numérica que una imagen no puede responder.

**De ahí la regla que sí queda operativa**, y está escrita en `docs/REVISION_VISUAL.md` §0: *`fullPage` sirve para revisar contenido, no para detectar desbordamiento; para eso se usa `npm run desborde`.* Cada herramienta responde a una pregunta distinta, y **confundir las dos preguntas es el fallo, no usar la herramienta equivocada por descuido.**

Esto no es una anécdota de descuidos sueltos. Es la misma clase de defecto un nivel más adentro: **un mecanismo funcionando correctamente dentro de un alcance que nunca declaró**, salvo que aquí el mecanismo es el que se usa para detectar precisamente eso. El cuarto caso lo lleva un paso más allá: el alcance no declarado **no era de ningún mecanismo, era el espacio entre dos** —lo que dice el documento y lo que pinta el navegador—, y ese hueco no aparece en el alcance de nadie. Y el quinto cierra la escalada: ahí el alcance no declarado **era el de la propia herramienta de mirar** — `fullPage` tiene un dominio de validez perfectamente definido y nadie lo había escrito, así que se usó para una pregunta que no contesta, durante fases enteras, en verde. Y tiene una consecuencia incómoda: la mutación, que es la práctica en la que más confía este proyecto, **no se audita a sí misma**. Una mutación que sobrevive puede significar que falta un test, o puede significar que la mutación no rompía nada — y los dos casos se ven exactamente igual en la consola.
