...
def crear_analisis(inp: AnalisisInput, db: Session = Depends(get_db),
    ...
    analisis_id, resultado = analisis_service.crear_analisis(db, inp, quien=user.email)
...
def listar(limit: int = 50, db: Session = Depends(get_db),
    ...
    return analisis_service.listar_analisis(db, limit)
...
def informe(analisis_id: str, db: Session = Depends(get_db),
            _u: models.Usuario = Depends(get_current_user)) -> str:
    a = analisis_service.obtener_analisis(db, analisis_id)
    if not a:
        raise HTTPException(404, "Análisis no encontrado")
...
def informe_pdf(analisis_id: str, db: Session = Depends(get_db),
                _u: models.Usuario = Depends(get_current_user)) -> Response:
    a = analisis_service.obtener_analisis(db, analisis_id)
    if not a:
        raise HTTPException(404, "Análisis no encontrado")
